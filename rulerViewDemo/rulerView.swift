...
class rulerView: UIView {
        ...
        for var i:CGFloat = 0; i < s+1; i++  {//一个i代表一毫米
            ...
            if(i%5==0){
                if(i%10==0){//10mm处
                    ...
                    //文字
                    var cm:UILabel = UILabel(frame: CGRectMake(40, i/lengthOfPer+10, 40, 20))
                    cm.text = "\(i/10)cm"
    ...
    override func updateConstraints() {
        ...
        dispatch_once(&StaticStruct.predicate){ () -> Void in
            /********************************初始化工作只能执行一次********************************/
            self.btn = UIButton()
            self.btn?.setTranslatesAutoresizingMaskIntoConstraints(false)
            self.btn?.backgroundColor = UIColor.blackColor()
            self.addSubview(self.btn!)
            
            self.btn2 = UIButton()
            self.btn2?.setTranslatesAutoresizingMaskIntoConstraints(false)
            ...
            var panGestureRecognizer:UIPanGestureRecognizer=UIPanGestureRecognizer(target: self, action: "handlePan:")
